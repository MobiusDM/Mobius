...
import (
	...
	"github.com/notawar/mobius/backend/server/archtest"
)
...
func TestAllAndroidPackageDependencies(t *testing.T) {
	t.Parallel()
	archtest.NewPackageTest(t, "github.com/notawar/mobius/backend/server/mdm/android...").
		OnlyInclude(regexp.MustCompile(`^github\.com/mobiusmdm/`)).
		WithTests().
		IgnoreXTests("github.com/notawar/mobius/backend/server/mobius"). // ignore mobius_test package
		IgnorePackages(
			"github.com/notawar/mobius/backend/server/datastore/mysql/common_mysql...",
			"github.com/notawar/mobius/backend/server/service/externalsvc", // dependency on Jira and Zendesk
			"github.com/notawar/mobius/backend/server/service/middleware/auth",
			"github.com/notawar/mobius/backend/server/service/middleware/authzcheck",
		...
		ShouldNotDependOn(
			...
			"github.com/notawar/mobius/backend/server/datastore...",
		)
}
